...
(function () {
  ...
  var state = {
    ...
    unsubscribe: null,
    onChange: null    // app.jsが登録するUI更新コールバック
  };
  ...
  function migrateLocalToCloud() {
    var local;
    try { local = JSON.parse(localStorage.getItem(STATS_KEY)) || {}; }
    catch (e) { local = {}; }
    ...
    return db.runTransaction(function (tx) {
      return tx.get(ref).then(function (snap) {
        ...
        tx.set(ref, { stats: stats, updatedAt: Date.now() }, { merge: true });
      });
    }).then(function () {
  ...
  window.STSync = {
    ...
    setOnChange: function (cb) { state.onChange = cb; },
    ...
    resetAll: function () {
      state.stats = {};
      docRef().set({ stats: {}, updatedAt: Date.now() });
    }
  };
